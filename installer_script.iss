...
#define MyAppExeName "DeepSeek-Tool.exe"
...
begin
  ...
  begin
    ...
    ForceDirectories(ExpandConstant('{commonappdata}\{#MyAppName}\logs'));
    // Garante permissões de escrita para usuários
    Exec('icacls.exe', ExpandConstant('"{commonappdata}\{#MyAppName}" /grant Users:(OI)(CI)F'), '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  end;
end; 